...
COMPRESSED_MODEL_PATH = './model/BC_model.zip'
EXTRACTED_MODEL_PATH = './model/BC.h5'

def extract_model_zip(zip_path, extract_to):
...
if not os.path.isfile(EXTRACTED_MODEL_PATH):
    if os.path.isfile(COMPRESSED_MODEL_PATH):
        extract_model_zip(COMPRESSED_MODEL_PATH, './model')
    else:
        st.error("Compressed model file not found.")

model = load_model_safe(EXTRACTED_MODEL_PATH)
